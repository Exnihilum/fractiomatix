...
public class FrontalMatrix {
	...
	final static int FM_ALLOCBLOCK = 16, FAILED_VECSIZE = 2;
	final static boolean EXTRA_FM_SPACE = false;
	static double pivotCritFactor = 0.1;						// pivotCritFactor * candidate pivot will elect candidate if > than current pivot
	...
	int nFailed = 0, nPivots = 0, nContribR = -1, nContribC = -1;
	...
	double[] data, idata;										// real and complex data arrays, linear layout
	...
	short childFlag = -1;										// child type flag for add() method
	...
	public FrontalMatrix(int p, int r, int c, int nContribR, int nContribC, int nExtraR, int nExtraC) {
		this.pivot = p;
		this.r = r; this.c = c;
		if (EXTRA_FM_SPACE) {					// allocate with space for at least one failed pivot and 1.25x extra space
			rTot = r + r/4 + 1;	 cTot = c + c/4 + 1;
		} else { rTot = r; cTot = c; }
		this.nContribR = nContribR; this.nContribC = nContribC;
		this.nExtraR = nExtraR; this.nExtraC = nExtraC;
		iTB = rTot * cTot;
		// additionally allocate space for global indexation of row & column and a temporary row/column assembly buffer
	...
	public FrontalMatrix(NSPMatrix A, int pivot, int nExtraP, int[][] gContribC, int[][] gContribR, int[][] gExtraC, int[][] gExtraR) {
		...
		if (gExtraR[1] != null) { this.nExtraR = gExtraR[1][0]; this.extraR = gExtraR; }
		if (gExtraC[1] != null) { this.nExtraC = gExtraC[1][0]; this.extraC = gExtraC; }
		this.nPivots = 1;

		int offH = A.pivotNsp[pivot].offH + 1, offV = A.pivotNsp[pivot].offV + 1;
		nContribR = gContribR[1][0] - 1;									// the total final dimensions of contribution block (minus pivot)
		nContribC = gContribC[1][0] - 1;
		...
		int[] contribC = gContribC[0], offset = gContribC[1];
		int j = offH, jO = 7, jIdx = nExtraP;
		int nContribCP = gContribC[1][2] + offH - 1, nContribRP = gContribR[1][2] + offV - 1;
		//idxCR[jIdx++] = pivot;
		...
		int i = offV, iO = 7,  iIdx = cTot + nExtraP;
		idxCR[iIdx++] = pivot;
...
//		}
	...
	public void add(FrontalMatrix fmC, int[][] gContribC, int[][] gContribR) {
			...
			if (fmC.nFailed > 0) {
				...
				while (jExC < jExCEnd && jExP < jExPEnd) {
					if (failedP[jExC3] == idxCR[jExP]) {						// check if child's failed global column == parent's extras global column
						int iC = cTotC + failedP[jExC3 + 1], iP = cTot;			// offsets to idxCR's frontal row indices of child & parent
	...
	public void add2(FrontalMatrix fmC, int[][] gContribC, int[][] gContribR) {
		...
		int cTotC = fmC.cTot,  skipExPiC = fmC.nExtraP + fmC.nPivots, skipExPiP = nExtraP + nPivots;
		int iCEnd = cTotC + fmC.r, iPEnd = cTot + r;
		int iC = cTotC + skipExPiC, iP = nExtraP + cTot;
		int idxP = idxCR[iP], idxC = idxCRC[iC];
		
		boolean execMixCodes = false;
		while (iC < iCEnd && iP < iPEnd) {
			if (idxP < idxC)		idxP = idxCR[++iP]; 
			else if (idxP > idxC)	idxC = idxCRC[++iC];
			else {
	...
	public void decomposeLU2(int parentLU, int[][] mutator) {
		double vPivot;
		for (int i = 0, iTot = nExtraP + nPivots; i < iTot; i++) {	// iterate over every pivot (including extra pivots)
			...
			if (intraPivoting(i, mutator)) {						// check numerical stability of current pivot, do pivoting of necessary
	...
	public boolean intraPivoting(int p, int[][] mutator) {
		...
		double pivotTest = pivot / pivotCritFactor, vMax = pivotTest;
		int p2 = p + 1, iPivots = nExtraP + nPivots;
		int idT = p * (cTot + 1) + cTot, iTEnd = r * cTot, iMax = -1;
		...
		if (iMax >= 0 || pivot <= pivotFailFactor) {
...
//				mutator[0][idxCR[p]] = idxCR[iMax];
			...
			vMax = pivotFailFactor;
			// DEBUG: this loop checks pivot's submatrix INCLUSIVE (i>=p & j>p) the row of the pivot
//			int jMax = p;
//			for (int i = p, jP = p * (cTot + 1), jAdd = cTot - iPivots; i < iPivots; i++, jP += jAdd)
...
//					}
			// DEBUG: this loop checks STRICTLY the submatrix (i>p & j>p) of the pivot
			int jMax = p;
			...
			if (vMax <= pivotFailFactor) return false;								// if nothing above pivot fail factor found, fail at this pivot
...
//				mutator[0][idxCR[p]] = idxCR[iMax];
	...
	private void extendRowData(double[] dS, double[] dD, int jS, int jD) {
		int m = 0;
		while (true) {
			switch (mixCode[m++]) {
			case 0:		jD+=4;																		// 0 0 0 0
			case 1:		jD+=3; dD[jD++]=dS[jS++];													// 0 0 0 1
			case 2:		jD+=2; dD[jD]=dS[jS++]; jD+=2;												// 0 0 1 0
			case 3:		jD+=2; dD[jD]=dS[jS++]; dD[jD]=dS[jS++];									// 0 0 1 1
			case 4:		jD++; dD[jD]=dS[jS++]; jD+=3;												// 0 1 0 0
			case 5:		jD++; dD[jD]=dS[jS++]; jD+=2; dD[jD++]=dS[jS++];							// 0 1 0 1
			case 6:		jD++; dD[jD++]=dS[jS++]; dD[jD]=dS[jS++]; jD+=2;							// 0 1 1 0
			case 7:		jD++; dD[jD]=dS[jS++]; dD[jD++]=dS[jS++]; dD[jD++]=dS[jS++];				// 0 1 1 1
			case 8:		dD[jD]=dS[jS++]; jD+=4;														// 1 0 0 0
			case 9:		dD[jD]=dS[jS++]; jD+=3; dD[jD++]=dS[jS++];									// 1 0 0 1
			case 10:	dD[jD]=dS[jS++]; jD+=2; dD[jD]=dS[jS++]; jD+=2;								// 1 0 1 0
			case 11:	dD[jD]=dS[jS++]; jD+=2; dD[jD++]=dS[jS++]; dD[jD++]=dS[jS++];				// 1 0 1 1
			case 12:	dD[jD++]=dS[jS++]; dD[jD]=dS[jS++]; jD+=3;									// 1 1 0 0
			case 13:	dD[jD++]=dS[jS++]; dD[jD]=dS[jS++]; jD+=2; dD[jD++]=dS[jS++];				// 1 1 0 1
			case 14:	dD[jD++]=dS[jS++]; dD[jD++]=dS[jS++]; dD[jD]=dS[jS++]; jD+=2;				// 1 1 1 0
			case 15:	dD[jD++]=dS[jS++]; dD[jD++]=dS[jS++]; dD[jD++]=dS[jS++]; dD[jD++]=dS[jS++];	// 1 1 1 1
			}
		}
	}
	...
	private void mixRowData(double[] dC, double[] dP, int jC, int jP) {
		...
		while (true) {
			...
			case -1:		return;
			default:
				throw new InvalidParameterException("FrontalMatrix.mixRowData(): Invalid mixCode encountered.");
			}
		}
	}
	...
	public String toString() {		
		...
		sb.append("Frontal matrix " + pivot + ", " + r + "x" + c + " (" + rTot + "x" + cTot + "):\n");

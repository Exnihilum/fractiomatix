...
public class CSRMatrix extends Matrix {
	...
	public double[] getData() {
		...
		if (nA == 0) return data;		// CSR matrix was empty
		for (int r = 0; r < M; r++) {
			int rN = r * N;
			int iar1 = IA[r + 1], nrv = iar1 - IA[r];
	...
	public void putData(double[] data) {
		...
		for (int j = 0; j < M; j++) {
			...
			int c = 0;
			for (int i = 0; i < N; i++)
				if (data[jN + i] > Matrix.ROUNDOFF_ERROR || data[jN + i] < -Matrix.ROUNDOFF_ERROR) c++;
			
			// assemble compact indexing column IA
			IA[j + 1] = IA[j] + c;
		}

		// gather up all non-zero matrix members in A and column indexes of all non-zero matrix members in JA
		nA = IA[nIA - 1];
		A = new double[nA];
		JA = new int[nA];
		int c = 0;
		...
		for (int j = 0; j < M; j++) {
			int jN = j * N;
			for (int i = 0; i < N; i++)
				if (data[jN + i] > Matrix.ROUNDOFF_ERROR || data[jN + i] < -Matrix.ROUNDOFF_ERROR) {
					A[c] = data[jN + i];
					JA[c++] = i;
				}
		}
	...
	public static boolean equal(CSRMatrix S, CSRMatrix T) {
		...
		for (int i = 0; i < S.M; i++) {
			...
			while (jS < nextIA_S && jT < nextIA_T) {
				...
				else if (S.A[jS++] != T.A[jT++]) return false;
			}
	...
	public static CSRMatrix add(CSRMatrix S, CSRMatrix T)							{ return addSub(S, T, false); }
	public static CSRMatrix subtract(CSRMatrix S, CSRMatrix T, boolean subtract)	{ return addSub(S, T, true); }
	...
	public static CSRMatrix addSub(CSRMatrix S, CSRMatrix T, boolean subtract) {
		...
		String newname;
		if (Matrix.DEBUG_LEVEL > 1) newname = "(" + S.name + (subtract ? "-" : "+") + T.name + ")";
		else						newname = (subtract ? "S" : "D") + nameCount++;
		CSRMatrix U = new CSRMatrix(newname, S.M, S.N);
		...
		U.bitImage = S.bitImage.clone(U);
		U.bitImage.or(S.bitImage);
		return U;
	}
	...
	public static CSRMatrix multiply(CSRMatrix S, CSRMatrix T) {
		...
		if (S.N != T.M)
			throw new RuntimeException("CSRMatrix.multiply(): Nonmatching matrix dimensions.");

		// results stored in new CSRMatrix U
		String newname;
		if (Matrix.DEBUG_LEVEL > 1) newname = "(" + S.name + "." + T.name + ")";
		else						newname = "P" + nameCount++;
		CSRMatrix U = new CSRMatrix(newname, S.M, T.N);

		// create new bitImage, we will fill it up dynamically. Note down what type of bitimage it is
		boolean bitImage8x8 = false;
		int bitSets = 0;
		if (S.M < 9 && T.N < 9)  bitImage8x8 = true;
		else {
		...
		int[] SIA = S.IA, TIA = T.IA, SJA = S.JA, TJA = T.JA;
		double[] SA = S.A, TA = T.A;
		...
		// and added to the result matrix row that is = the column of the multiplicator value

		// for every row in first matrix S
		for (int iS = 0; iS < S.M; iS++) {
			...
			for (int k=0; k < T.N; k++) {
				if (tmp[k] < -ROUNDOFF_ERROR || tmp[k] > ROUNDOFF_ERROR) {
					Aop[l] = tmp[k];
					JAop[l++] = k;
					IA1[iIA]++;
	...
	public String toString() {
		...
		System.out.println("CSR matrix: " + this.name);
		sb.append("A:  [");
